...
class DbPlan(db_plan.DbPlan):
    def __init__(self):
        ...
        super().__init__()
        self.ui_text_prefix = (
            "PaywareCustomEndpoint"  # Will be used for all self.ui_text_elements
        )
        ...
        # self.upde_allow_user_edit = True  # Sets the Allow User Edit column on the Q2_UserPropertyDataElements table entry for this extension
        # self.upde_allow_user_view = True  # Sets the Allow User View column on the Q2_UserPropertyDataElements table entry for this extension
        # self.upde_allow_cust_view = True  # Sets the Allow Cust View column on the Q2_UserPropertyDataElements table entry for this extension

        # self.api_stored_procs = [db_plan.ApiStoredProc()]
        # self.audit_actions = [db_plan.AuditAction()]
        # self.audit_category = [db_plan.AuditCategory()]
        # self.data_feeds = [db_plan.DataFeed()]
        # self.disclaimers = [db_plan.Disclaimer()]
        # self.gt_data_elements = [db_plan.GTDataElement()]
        # self.gt_flavor = [db_plan.GtFlavor()]
        # self.host_account_data_elements = [db_plan.HostAccountDataElement()]
